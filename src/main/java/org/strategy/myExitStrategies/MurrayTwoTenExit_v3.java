...
public class MurrayTwoTenExit_v3 extends Strategy {
    final Double murrayRange = 38.12;
    final double proportionOfFirstClose = 0.5;
...
//            }
            ...
            tradeEngine.setStopLoss(order, murrayLevels[1] - 0.0003);
            tradeEngine.setTakeProfit(order, getNextMurrayLevel(order.openPrice+0.00038,-0.00003,true));
...
//            tradeEngine.logOrderActivity(order,"TARGET","takeProfitTarget: "+order.takeProfitTarget);
            ...
            tradeEngine.setStopLoss(order, murrayLevels[11] + 0.0003);
            tradeEngine.setTakeProfit(order, getNextMurrayLevel(order.openPrice-0.00038,0.00003,false));
...
//        atrBasedTrailingBias= atrValue / 3.0;
//
...
//        System.out.println(symbol+" "+series.getCurrentTime());
    ...
    private int getMurrayRange(double value) {
    ...
    private double getNextMurrayLevel(double currentPrice, double bias, boolean upper) {
        int murrayLevel;
