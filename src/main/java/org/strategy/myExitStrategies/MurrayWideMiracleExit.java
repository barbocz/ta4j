...
public class MurrayWideMiracleExit extends Strategy {
...
//        System.out.println(symbol+" "+series.getCurrentTime());
    ...
    void setTakeProfitStoploss(Order order, double currentPrice, boolean closerStopNeeded) {
        ...
        int murrayRange;
        double range = 0.00153;
        if (closerStopNeeded) range=0.00038;
        if (order.type == Order.Type.BUY) {

            murrayRange = getMurrayRange(currentPrice - range);
            if (murrayRange < 0) order.stopLoss = currentPrice - range;
            else
                order.stopLoss = murrayMathIndicators[murrayRange].getValue(tradeEngine.currentBarIndex).doubleValue() - 0.00038;
            if (order.phase == 1 && order.stopLoss < order.openPrice) order.stopLoss = order.openPrice + 0.00015;


            if (!closerStopNeeded) {
                murrayRange = getMurrayRange(currentPrice + 0.0006);
                if (murrayRange < 0 || murrayRange == 12) order.takeProfit = currentPrice + 0.001;
                else
                    order.takeProfit = murrayMathIndicators[murrayRange + 1].getValue(tradeEngine.currentBarIndex).doubleValue() - 0.00008;
            }
            ...
            if (murrayRange < 0 || murrayRange == 12) order.stopLoss = currentPrice + range;
                ...
                order.stopLoss = murrayMathIndicators[murrayRange + 1].getValue(tradeEngine.currentBarIndex).doubleValue() + 0.00038;
            if (order.phase == 1 && order.stopLoss > order.openPrice) order.stopLoss = order.openPrice -  0.00015;


            if (!closerStopNeeded) {
                murrayRange = getMurrayRange(currentPrice - 0.001);
                if (murrayRange < 0) order.takeProfit = currentPrice - 0.0006;
                else
                    order.takeProfit = murrayMathIndicators[murrayRange].getValue(tradeEngine.currentBarIndex).doubleValue() + 0.00008;
            }
        }
    }
