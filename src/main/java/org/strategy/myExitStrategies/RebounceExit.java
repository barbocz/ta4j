...
//            order.takeProfit=murrayLevels[4]-0.00005;
            order.stopLoss=murrayLevels[0]-2*murrayHeight;
        } else {
//            order.takeProfit=murrayLevels[8]+0.00005;
            order.stopLoss=murrayLevels[12]+2*murrayHeight;
        }
...
//        System.out.println("onTickEvent------------- "+tradeEngine.timeSeriesRepo.bid);
    ...
    public void onBarChangeEvent(int timeFrame) throws Exception{
        ...
        for (Order order : tradeEngine.openedOrders) {
            ...
            if (order.type == Order.Type.BUY && signalLevel>1) {
            ...
            } else if (order.type == Order.Type.SELL && signalLevel<-1) {
