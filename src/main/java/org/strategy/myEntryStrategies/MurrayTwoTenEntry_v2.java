...
public class MurrayTwoTenEntry_v2 extends Strategy {
...
//    Rule orderConditionRule;
    ...
    public void init() {
        ...
        ZoloIndicator zoloIndicatorUp = new ZoloIndicator(tradeEngine.series, 8, 8, true);
        ZoloIndicator zoloIndicatorDown = new ZoloIndicator(tradeEngine.series, 8, 8, false);
    ...
    public void onTickEvent() throws Exception {
        ...
        if (tradeEngine.timeSeriesRepo.ask > sellLimit) {
            if (tradeEngine.backtestMode) {
                order = Order.sell(orderAmount, sellLimit, tradeEngine.series.getCurrentTime());
        ...
        if (tradeEngine.timeSeriesRepo.bid < buyLimit) {
            if (tradeEngine.backtestMode) {
                order = Order.buy(orderAmount, buyLimit, tradeEngine.series.getCurrentTime());
                tradeEngine.onTradeEvent(order);
            } else {
...
//kcL.getValue(tradeEngine.currentBarIndex).doubleValue()<buyLimit ||
