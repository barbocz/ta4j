...
public class MurrayMiracleEntry_M30 extends Strategy {
    ...
    public void init() {
        ...
        for (int i = 0; i < 13; i++) {
            murrayMathIndicators[i] = new MurrayMathFixedIndicator(tradeEngine.series, i, 152.0);
        }
        ...
        MurrayChangeIndicator murrayChangeIndicator = new MurrayChangeIndicator(tradeEngine.series,152);
...
//        ruleForBuy = ruleForBuy.and(new UnderIndicatorRule(murrayChangeIndicator, 5.5));
    ...
    public void onBarChangeEvent(int timeFrame) throws Exception {
        sellLimit = 0.0;
        buyLimit = Double.MAX_VALUE;
        if (ruleForSell.isSatisfied(tradeEngine.series.getCurrentTime()) && tradeEngine.series.getCurrentBar().getVolume().doubleValue()<7000) {
            sellLimit = tradeEngine.series.getCurrentBar().getClosePrice().doubleValue() - 0.00016;
        }
        else if (ruleForBuy.isSatisfied(tradeEngine.series.getCurrentTime()) && tradeEngine.series.getCurrentBar().getVolume().doubleValue()<7000) {
            buyLimit = tradeEngine.series.getCurrentBar().getClosePrice().doubleValue() + 0.00016;
        }
    }
